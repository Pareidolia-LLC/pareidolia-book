...
<style>
  /* =====================================================================
     PAREIDOLIA - the painted edition.

     Two references, held together on purpose:

     FRAZETTA   warm earth pulled out of deep shadow. Umber ground, ochre
                torchlight, oxblood, bone. Light pools in one place and
                everything else falls away. Nothing is grey.
     TARTAKOVSKY  flat graphic blocking over that ground. Hard edges, no
                gradients on type, colour laid down in slabs, and motion
                that cuts rather than fades - the panel wipe is the whole
                transition, and it is meant to be seen.

     One sheet. No colourway, no override layers.
     ===================================================================== */

  :root{
    /* ground: umber pulled down to near-black, never neutral grey */
    --bg:#14110E; --panel:#1D1813; --panel-2:#272019; --line:#3E3428;
    --ink:#F0E6D2; --muted:#A89377; --faint:#6F6250;
    /* torchlight */
    --accent:#E0902F; --accent-soft:rgba(224,144,47,.14);
    --accent-2:#A63A22;
    --up:#93A857; --down:#C4442C; --warn:#E0902F; --compare:#6B8CA6;
    --slate:#6B8CA6; --grid:rgba(240,230,210,.07);
    /* aliases the chart helper and older markup still read */
    --gold:#E0902F; --gold-lift:#F0AE55; --paper:#1D1813;
    --silver:#A89377; --silver-lift:#C9B394;
    --engrave:#E0902F; --label:#E0902F;
    --plate:#14110E; --plate-edge:#3E3428; --plate-rule:#E0902F; --plate-ink:#A89377;
    --pinstripe:rgba(240,230,210,.022); --pinstripe-gold:rgba(224,144,47,.05);
    color-scheme:dark;
  ...
  *{box-sizing:border-box}
  ...
  /* torchlight: one warm source top-left, everything else falls into shadow */
  .wrap::before{content:""; position:fixed; inset:0; z-index:0; pointer-events:none;
    background:
      radial-gradient(120% 90% at 12% -10%, rgba(224,144,47,.13), transparent 58%),
      radial-gradient(90% 70% at 100% 105%, rgba(166,58,34,.10), transparent 60%)}
  /* canvas tooth */
  .wrap::after{content:""; position:fixed; inset:0; z-index:0; pointer-events:none; opacity:.5;
    background:repeating-linear-gradient(122deg,
      var(--pinstripe) 0 1px, transparent 1px 4px)}
  .sheet{max-width:1180px; margin:0 auto; position:relative; z-index:1}

  /* ---------- the wipe: a hard cut, three slabs ---------- */
  .wipe{position:fixed; inset:0; z-index:60; pointer-events:none;
    display:grid; grid-template-rows:repeat(3,1fr)}
  .wipe i{display:block; transform:scaleX(0); transform-origin:left center; will-change:transform}
  .wipe i:nth-child(1){background:var(--accent)}
  .wipe i:nth-child(2){background:var(--accent-2)}
  .wipe i:nth-child(3){background:var(--ink)}
  @keyframes slab{
  ...
  .wipe.run i{animation:slab .66s var(--cut) both}
  .wipe.run i:nth-child(2){animation-delay:.05s}
  .wipe.run i:nth-child(3){animation-delay:.10s}

  /* panels arrive with a lateral push, staggered - the smear after the cut */
  @keyframes push{from{opacity:0; transform:translateX(-26px)}to{opacity:1; transform:none}}
  .panel{display:none}
  .panel.active{display:block}
  .panel.active>section,.panel.active>div>section,.panel.active>div{
    animation:push .42s var(--ease-out) both}
  ...
  .panel.active>section:nth-child(n+5){animation-delay:.18s}
  ...
  .masthead{display:flex; align-items:flex-end; justify-content:space-between;
    gap:16px; flex-wrap:wrap; padding:6px 0 10px}
  .mark{font-family:var(--display); font-weight:700;
    font-size:clamp(44px,9vw,104px); line-height:.82; letter-spacing:-.022em;
    text-transform:uppercase; color:var(--ink);
    text-shadow:0 0 44px rgba(224,144,47,.16)}
  .mark .dot{color:var(--accent-2)}
  .tag{font-family:var(--serif); font-size:14.5px; font-style:italic;
    color:var(--muted); max-width:52ch; line-height:1.6; margin:8px 0 0}
  .rosette,.om{display:none}
  .dateline{display:flex; gap:8px 26px; flex-wrap:wrap; align-items:center;
    font-family:var(--mono); font-size:9.5px; letter-spacing:.22em;
    text-transform:uppercase; color:var(--faint);
    border-top:5px solid var(--accent); border-bottom:1px solid var(--line);
    padding:11px 0}
  ...
  /* ---------- tabs: slabs, not links ---------- */
  .tabs{display:flex; gap:2px; position:sticky; top:0; z-index:7;
    background:var(--bg); overflow-x:auto; scrollbar-width:none; margin:0 0 38px;
  ...
  .tab{appearance:none; border:0; cursor:pointer; font:inherit; font-family:var(--display);
    ...
    color:var(--faint); background:var(--panel); padding:14px 24px 12px;
    white-space:nowrap; position:relative; transition:color .16s, background .16s}
  .tab:hover{color:var(--ink); background:var(--panel-2)}
  .tab.active{color:#14110E; background:var(--accent)}
  .tab .fk{font-family:var(--mono); font-size:10px; margin-right:10px; opacity:.65; font-weight:400}
  .tabink{display:none}
  ...
  /* ---------- section heads: a struck slab ---------- */
  section{margin-top:clamp(36px,5.2vw,66px)}
  .panel.active>section:first-child,.panel.active>div:first-child section:first-child{margin-top:0}
  .eyebrow{display:inline-block; margin:0 0 10px; background:var(--accent-2); color:var(--ink);
    font-family:var(--mono); font-size:9.5px; font-weight:700; letter-spacing:.24em;
    text-transform:uppercase; padding:5px 11px}
  h2{font-family:var(--display); font-weight:700; font-size:clamp(24px,3.2vw,38px);
    letter-spacing:-.018em; line-height:1.06; margin:0 0 6px; color:var(--ink);
    text-transform:uppercase}
  h3{font-family:var(--display); font-size:15px; font-weight:700; letter-spacing:.02em;
  ...
  .stat{background:var(--panel); padding:22px 22px 20px; border-left:5px solid var(--accent)}
  ...
  .stat .v{font-family:var(--display); font-variant-numeric:tabular-nums;
    font-size:clamp(40px,5.8vw,66px); font-weight:700; letter-spacing:-.035em;
  ...
  .stat .v.pos ~ .k,.stat .v.neg ~ .k{color:inherit}
  .asofline{font-family:var(--mono); font-size:9.5px; letter-spacing:.2em;
    text-transform:uppercase; color:var(--faint); margin:14px 0 0}
  .asofline b{color:var(--muted); font-weight:700}

  /* ---------- chart ---------- */
  .chart-card{background:var(--panel); border:0; border-left:5px solid var(--accent-2);
    padding:20px 20px 14px; margin-top:14px; position:relative;
    --accent:var(--gold); --slate:var(--compare)}
  .chart-head{display:flex; justify-content:space-between; align-items:baseline;
    gap:14px; flex-wrap:wrap}
  .chart-head .sub{font-family:var(--mono); font-size:10px; letter-spacing:.14em;
    text-transform:uppercase; color:var(--faint)}
  ...
  .tlv{appearance:none; background:var(--panel-2); border:0; cursor:pointer;
    ...
    color:var(--faint); padding:7px 13px; transition:.14s}
  .tlv:hover{color:var(--ink); background:var(--line)}
  .tlv.active{background:var(--accent); color:#14110E; font-weight:700}
  .marks{display:flex; gap:24px; flex-wrap:wrap; margin-top:14px; padding-top:12px;
  ...
  .marks b{color:var(--ink); font-weight:700; font-variant-numeric:tabular-nums}
  .swatch,.sw{width:9px; height:9px; display:inline-block}
  .ctip{position:absolute; display:none; pointer-events:none; z-index:8;
    transform:translate(-50%,-100%); background:var(--ink); color:#14110E;
    padding:5px 10px; font-family:var(--mono); font-size:10.5px; white-space:nowrap; font-weight:600}
  .ctip .tdate{opacity:.65; margin-right:10px}
  .ctip .tval{font-weight:700}

  /* ---------- after-action ---------- */
  .rc-head{display:flex; align-items:center; gap:22px; flex-wrap:wrap; margin:16px 0 18px}
  .grade{font-family:var(--display); font-weight:700; font-size:60px; line-height:.86;
    letter-spacing:-.04em; padding:14px 24px; background:var(--panel);
    border-left:5px solid currentColor}
  .rc-head .wk{font-family:var(--mono); font-size:10.5px; letter-spacing:.16em;
  ...
  .rbadge{display:inline-block; font-family:var(--mono); font-size:8.5px;
    ...
    border:1px solid var(--line); padding:3px 9px; margin-top:8px}

  .dials{display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:2px}
  .dial{background:var(--panel); padding:17px 18px 16px; border-left:5px solid var(--line)}
  .dial.pass{border-left-color:var(--up)}
  .dial.warn{border-left-color:var(--warn)}
  .dial.fail{border-left-color:var(--down)}
  .dial .dk{font-family:var(--mono); font-size:9px; letter-spacing:.2em;
    text-transform:uppercase; color:var(--faint)}
  .state{display:inline-block; font-family:var(--mono); font-size:8.5px; font-weight:700;
    letter-spacing:.19em; text-transform:uppercase; padding:3px 9px; margin-top:10px; color:#14110E}
  ...
  .dial .dv{font-family:var(--display); font-variant-numeric:tabular-nums;
    font-size:27px; font-weight:700; letter-spacing:-.02em; margin:11px 0 6px; color:var(--ink)}
  .dial .rule{font-family:var(--serif); font-size:12.5px; color:var(--faint); line-height:1.55}

  .rc-note{margin-top:18px; font-family:var(--serif); font-size:16px; line-height:1.78;
  ...
  .rbasis{font-family:var(--serif); font-style:italic; font-size:13.5px; color:var(--faint);
    margin-top:13px; line-height:1.68; max-width:76ch; border-left:2px solid var(--line); padding-left:15px}

  .hlabel{font-family:var(--mono); font-size:9.5px; letter-spacing:.2em;
    text-transform:uppercase; color:var(--faint); margin:28px 0 4px}
  .hnote{font-family:var(--serif); font-size:13px; color:var(--faint); margin:0 0 12px; font-style:italic}
  ...
  .gcard{flex:0 0 auto; min-width:84px; background:var(--panel); border:0; cursor:pointer;
    font:inherit; color:inherit; padding:12px 12px 10px; text-align:center;
    border-bottom:3px solid transparent; transition:background .14s}
  .gcard:hover{background:var(--panel-2)}
  .gcard.active{background:var(--panel-2); border-bottom-color:var(--accent)}
  .gcard.recon{opacity:.6}
  .gcard.recon:hover,.gcard.recon.active{opacity:1}
  .gw{font-family:var(--mono); font-size:9px; letter-spacing:.12em;
    text-transform:uppercase; color:var(--faint)}
  .gg{font-family:var(--display); font-size:26px; font-weight:700; letter-spacing:-.03em;
  ...
  .grecon{font-family:var(--mono); font-size:7.5px; letter-spacing:.16em;
  ...
  .heat i:hover{transform:scale(1.25); position:relative; z-index:2}
  .heat-scale{display:flex; align-items:center; gap:11px; margin-top:14px;
  ...
  /* ---------- allocation ---------- */
  .bookgrid{display:grid; grid-template-columns:repeat(auto-fit,minmax(330px,1fr)); gap:38px}
  ...
  /* ---------- tables ---------- */
  .tablewrap{overflow-x:auto; margin-top:14px; background:var(--panel)}
  table{width:100%; border-collapse:collapse; font-family:var(--mono); font-size:11.5px}
  thead th{background:var(--accent-2); color:var(--ink); font-weight:700;
    font-size:9px; letter-spacing:.18em; text-transform:uppercase; text-align:left;
    padding:10px 13px; white-space:nowrap}
  td{padding:9px 13px; border-bottom:1px solid var(--line); color:var(--muted);
    font-variant-numeric:tabular-nums}
  tbody tr:last-child td{border-bottom:0}
  tbody tr:hover td{background:var(--panel-2); color:var(--ink)}
  th.r,td.r,.num.r{text-align:right}
  .tk{color:var(--ink); font-weight:700; letter-spacing:.07em}
  .chip{display:inline-block; font-family:var(--mono); font-size:8.5px;
    letter-spacing:.16em; text-transform:uppercase; padding:2px 8px;
    background:var(--panel-2); color:var(--faint)}
  .chip.wheel{background:var(--accent); color:#14110E; font-weight:700}
  .chip.dir{background:var(--compare); color:#14110E; font-weight:700}

  /* ---------- strategy cards ---------- */
  .cards{display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr));
    gap:2px; margin-top:16px}
  .appr{background:var(--panel); padding:22px; display:flex; flex-direction:column;
    border-top:5px solid var(--accent)}
  .appr:nth-child(2){border-top-color:var(--warn)}
  .appr:nth-child(3){border-top-color:var(--compare)}
  .appr h3{font-family:var(--display); font-size:17px; font-weight:700; letter-spacing:.01em;
    text-transform:uppercase; color:var(--ink); margin:0 0 12px; display:flex;
    align-items:baseline; gap:10px; flex-wrap:wrap}
  .appr .idx{color:var(--accent); font-family:var(--mono); font-size:11px; letter-spacing:.2em}
  .appr h3 .tag{font-family:var(--mono); font-size:8.5px; letter-spacing:.18em;
  ...
  .appr .ctl{margin-top:auto; padding-top:12px; border-top:1px solid var(--line);
    font-family:var(--mono); font-size:9px; letter-spacing:.16em;
    text-transform:uppercase; color:var(--faint)}
  .appr .ctl b{color:var(--accent); font-weight:700}

  /* ---------- mandate ---------- */
  .con{display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:2px; margin-top:16px}
  .con .box{background:var(--panel); padding:18px 20px; border-left:5px solid var(--line)}
  .con .box.ok{border-left-color:var(--up)}
  .con .box.no{border-left-color:var(--down)}
  .con .box h4{font-family:var(--mono); font-size:9px; letter-spacing:.2em;
  ...
  /* ---------- concepts ---------- */
  .cnav{display:flex; gap:2px; overflow-x:auto; scrollbar-width:none; margin-bottom:34px}
  ...
  .cbtn{appearance:none; background:var(--panel); border:0; cursor:pointer; font:inherit;
    ...
    color:var(--faint); padding:14px 20px; text-align:left; white-space:nowrap;
    display:flex; flex-direction:column; gap:6px; transition:.15s;
    border-bottom:3px solid transparent}
  .cbtn b{font-family:var(--display); font-size:15px; letter-spacing:.01em;
    color:var(--muted); text-transform:uppercase; font-weight:700}
  .cbtn:hover{background:var(--panel-2)}
  .cbtn.on{background:var(--panel-2); border-bottom-color:var(--accent)}
  .cbtn.on b{color:var(--accent)}
  .concept{display:none}
  .concept.active{display:block; animation:push .42s var(--ease-out) both}
  .fsview{display:none}
  ...
  .fs-kicker{font-family:var(--serif); font-style:italic; font-size:18px;
    line-height:1.62; color:var(--ink); max-width:62ch; margin:0 0 20px}
  .fs-note{font-family:var(--serif); font-size:14.5px; line-height:1.72; color:var(--faint);
    max-width:70ch; border-left:4px solid var(--accent-2); padding-left:18px; margin:20px 0}
  .fs-cover{font-family:var(--mono); font-size:9.5px; letter-spacing:.14em;
  ...
  .fs-fc{display:inline-flex; align-items:center; gap:8px; appearance:none; cursor:pointer;
    ...
    background:var(--panel); border:0; color:var(--faint); padding:6px 10px; transition:.14s}
  .fs-fc .sw{width:9px; height:9px; flex:none}
  .fs-fc:hover{background:var(--panel-2); color:var(--ink)}
  .fs-fc.active{background:var(--accent); color:#14110E; font-weight:700}
  .fs-search,.fs-q{font-family:var(--mono); font-size:11px; background:var(--panel);
    border:1px solid var(--line); color:var(--ink); padding:9px 12px; width:100%;
  ...
  table.fs-corr th{font-weight:700; color:var(--faint); font-size:9px; letter-spacing:.11em;
  ...
  .fs-untracked{font-family:var(--mono); font-size:9px; letter-spacing:.14em;
    text-transform:uppercase; color:var(--faint)}
  .fs-fc,.fs-tier{line-height:1.4}
  .vs-flag,.gs-flags span{display:inline-block; font-family:var(--mono); font-size:8px;
    letter-spacing:.12em; text-transform:uppercase; background:var(--panel-2);
    color:var(--faint); padding:1px 6px; margin:1px 2px 1px 0}
  .vs-flag.warn{background:var(--down); color:var(--ink)}
  ...
  .vs-score{font-family:var(--display); font-weight:700; font-size:15px; color:var(--accent)}
  ...
  .vs-ins{display:inline-block; font-family:var(--mono); font-size:8.5px;
    letter-spacing:.11em; padding:1px 7px; background:var(--panel-2); color:var(--faint)}
  .vs-ins.heavy{background:var(--accent); color:#14110E; font-weight:700}
  .vs-ins.absent{opacity:.55}
  .dist{margin:22px 0 6px}
  ...
  .dist .bars i{flex:1 1 0; background:var(--accent); opacity:.3; min-height:1px}
  ...
  .more{font-family:var(--mono); font-size:9.5px; letter-spacing:.13em;
  ...
  .statusbar{display:flex; position:fixed; left:0; right:0; bottom:0; z-index:20;
    gap:24px; flex-wrap:wrap; justify-content:center; background:var(--accent-2);
    padding:8px 14px; font-family:var(--mono); font-size:9px; letter-spacing:.18em;
    text-transform:uppercase; color:rgba(240,230,210,.72)}
  .statusbar b{color:var(--ink); font-weight:700}
  .statusbar .live{color:var(--up)}

  /* ---------- footer ---------- */
  footer{margin-top:64px; padding-top:22px; border-top:5px solid var(--accent)}
  footer p{font-family:var(--serif); font-size:13.5px; color:var(--faint);
    line-height:1.8; margin:0 0 10px; max-width:84ch}
  footer .meth{font-family:var(--mono); font-size:8.5px; letter-spacing:.18em;
    text-transform:uppercase; opacity:.75}

  /* ---------- stale bar ---------- */
  .stalebar{position:fixed; left:0; right:0; bottom:32px; z-index:40; background:var(--accent);
    color:#14110E; font-family:var(--mono); font-size:10px; letter-spacing:.14em;
  ...
  .stalebar button{font:inherit; margin-left:10px; background:none; cursor:pointer;
    border:2px solid #14110E; color:inherit; padding:3px 11px}
  ...
  @media (max-width:760px){
    .bookgrid{grid-template-columns:1fr; gap:28px}
    .grade{font-size:44px; padding:10px 17px}
    .bar{grid-template-columns:54px 1fr 48px; gap:11px}
    .statusbar{gap:12px; font-size:8px}
    .tab{padding:12px 16px 10px; font-size:13px}
  }
  @media (max-width:520px){
    ...
    .mark{font-size:clamp(34px,13vw,52px)}
  }
</style></head>
<body>
