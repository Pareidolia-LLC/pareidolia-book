...
<style>
  ...
  *{box-sizing:border-box} body{margin:0}
  ...
  canvas#curve{display:block; width:100%; height:290px; margin-top:6px}
...
<div class="wrap"><div class="sheet">
  ...
  <section aria-label="Cumulative return">
    ...
    <div class="chart-card">
      ...
      <canvas id="curve" role="img" aria-label="Cumulative time-weighted return, year to date."></canvas>
...
(function(){
  ...
  (function(){
    ...
    DATA.positions.forEach(function(p){
      ...
      row.innerHTML='<div class="name">'+p.t+'</div><div class="track"><div class="fill" style="background:'+col+'"></div></div><div class="pct">'+p.w.toFixed(1)+'%</div>';
      wrap.appendChild(row);
      var f=row.querySelector(".fill");
      requestAnimationFrame(function(){requestAnimationFrame(function(){f.style.transform="scaleX("+(p.w/max)+")";});});
    });
  })();
  ...
  var vals=CPS.map(function(x){return x*100;});
  var lo=Math.min.apply(null,vals), hi=Math.max.apply(null,vals);
  var yMin=Math.floor((lo-2)/5)*5, yMax=Math.ceil((hi+2)/5)*5;
  (function(){
    var m=document.getElementById("marks");
    m.innerHTML='<span><i class="swatch" style="background:var(--up)"></i>Peak <b>'+fmt(hi)+'</b></span>'+
  ...
  function ticks(){var o=[],seen={};for(var i=0;i<DATA.curve.dates.length;i++){var mo=DATA.curve.dates[i].slice(4,6);if(!seen[mo]){seen[mo]=1;o.push({i:i,label:MONTHS[parseInt(mo,10)-1]});}}return o;}
  function draw(p){
    ...
    for(var g=yMin; g<=yMax+0.001; g+=step){
      var y=Y(g); ctx.strokeStyle=(Math.abs(g)<0.001)?mut:grid; ctx.lineWidth=1;
  ...
  if(reduce){draw(1);}else{requestAnimationFrame(anim);}
