...
<style>
  /* =====================================================================
     PAREIDOLIA - one stylesheet, written once.

     Two registers, deliberately:
       DESK      near-black, dense, mono, amber. Numbers are the loudest
                 thing on the page and rules are heavy enough to read as
                 structure rather than decoration.
       EDITORIAL long-form research prose in The Lab and The Story gets a
                 serif, a real measure and generous leading, because those
                 are read rather than scanned.

     No ornament, no second colourway, no override layers. If something
     needs to change, change it here - there is nothing beneath it.
     ===================================================================== */

  :root{
    /* ground */
    --bg:#0A0A0B; --panel:#101012; --panel-2:#17171A; --line:#26262B;
    --ink:#EDEDF0; --muted:#8B8B94; --faint:#5B5B64;
    /* signal */
    --accent:#FFB000; --accent-soft:rgba(255,176,0,.13);
    --up:#3DE87A; --down:#FF5A45; --warn:#FFB000; --compare:#38CFE8;
    --slate:#6E6E78; --grid:rgba(237,237,240,.07);
    /* chart aliases kept for the drawing helper */
    --gold:#FFB000; --gold-lift:#FFC94D; --paper:#101012;
    --silver:#8B8B94; --silver-lift:#B6B6BE;
    --engrave:#FFB000; --label:#FFB000;
    --plate:#0A0A0B; --plate-edge:#26262B; --plate-rule:#FFB000; --plate-ink:#8B8B94;
    --pinstripe:rgba(237,237,240,.028); --pinstripe-gold:rgba(255,176,0,.05);
    color-scheme:dark;
  ...
  *{box-sizing:border-box}
  ...
  .wrap{background:var(--bg); color:var(--ink); font-family:var(--mono);
    min-height:100vh; padding:var(--pagepad); line-height:1.5;
    -webkit-font-smoothing:antialiased; position:relative}
  /* scanline. one job, very quiet */
  ...
  /* ---------- masthead: the name, set as a slab ---------- */
  .plate{background:none; border:0; padding:0}
  .masthead{display:flex; align-items:flex-end; justify-content:space-between;
    gap:18px; flex-wrap:wrap; padding:4px 0 12px}
  .mark{font-family:var(--mono); font-weight:700;
    font-size:clamp(38px,7.6vw,84px); line-height:.86; letter-spacing:-.03em;
    text-transform:uppercase; color:var(--ink)}
  .mark .dot{color:var(--accent)}
  /* .tag is overloaded: the masthead strapline, section subtitles, and a badge
     inside a strategy card. Only the strapline wants the wide-tracked caps. */
  .tag{font-family:var(--serif); font-size:14px; letter-spacing:0; text-transform:none;
    color:var(--faint); max-width:78ch; line-height:1.65; margin:6px 0 0}
  .masthead .tag{font-family:var(--mono); font-size:10.5px; letter-spacing:.2em;
    text-transform:uppercase; max-width:46ch; line-height:1.7}
  ...
  .tapetrack{display:inline-flex; gap:30px; white-space:nowrap; padding:7px 16px;
  ...
  .tapetrack{animation:crawl 72s linear infinite}
  @media (prefers-reduced-motion:reduce){ .tapetrack{animation:none} }

  /* ---------- tabs ---------- */
  .tabs{display:flex; gap:0; position:sticky; top:0; z-index:6;
    background:var(--bg); border-bottom:1px solid var(--line);
    overflow-x:auto; scrollbar-width:none; margin-bottom:34px}
  .tabs::-webkit-scrollbar{display:none}
  .tab{appearance:none; background:none; border:0; cursor:pointer; font:inherit;
    font-family:var(--mono); font-size:11px; font-weight:600; letter-spacing:.17em;
    text-transform:uppercase; color:var(--faint); padding:15px 20px 13px;
    white-space:nowrap; position:relative; transition:color .14s}
  .tab:hover{color:var(--ink)}
  .tab.active{color:var(--accent)}
  .tab .fk{color:var(--faint); margin-right:9px; font-weight:400}
  .tab.active .fk{color:var(--accent); opacity:.7}
  .tabink{position:absolute; bottom:-1px; height:2px; background:var(--accent);
    transition:transform .3s var(--ease-out), width .3s var(--ease-out); pointer-events:none}
  .tabs:not(.hasink) .tab.active::after{content:""; position:absolute; left:0; right:0;
  ...
  .panel{display:none}
  .panel.active{display:block; animation:pgIn .3s var(--ease-out) both}
  ...
  /* ---------- section heads ---------- */
  section{margin-top:clamp(34px,5vw,62px)}
  .panel.active>section:first-child,.panel.active>div:first-child section:first-child{margin-top:0}
  .eyebrow{display:flex; align-items:center; gap:12px; margin:0 0 6px;
    font-family:var(--mono); font-size:9.5px; font-weight:700; letter-spacing:.24em;
    text-transform:uppercase; color:var(--accent)}
  .eyebrow::after{content:""; flex:1; height:1px; background:var(--line)}
  h2{font-family:var(--mono); font-weight:700; font-size:clamp(19px,2.4vw,26px);
    letter-spacing:-.015em; margin:0 0 4px; color:var(--ink); text-transform:none}
  h3{font-family:var(--mono); font-size:11px; font-weight:700; letter-spacing:.16em;
    text-transform:uppercase; color:var(--muted); margin:0 0 8px}

  /* ---------- headline numbers: the loudest thing here ---------- */
  ...
  .stat .v{font-family:var(--mono); font-variant-numeric:tabular-nums;
    font-size:clamp(38px,5.6vw,62px); font-weight:700; letter-spacing:-.045em;
    line-height:1; margin:14px 0 0; color:var(--ink)}
  .stat .m{font-family:var(--mono); font-size:10px; letter-spacing:.06em;
  ...
  .chart-card{background:var(--panel); border:1px solid var(--line);
    padding:18px 18px 12px; margin-top:12px; position:relative;
  ...
  .tlviews{display:flex; gap:0; flex-wrap:wrap; margin-top:14px;
  ...
  .tlv.active{background:var(--accent); color:#0A0A0B; font-weight:700}
  ...
  .ctip .tdate{color:var(--faint); margin-right:10px}
  .ctip .tval{font-weight:700}

  /* ---------- weekly after-action ---------- */
  .rc-head{display:flex; align-items:center; gap:20px; flex-wrap:wrap; margin:14px 0 16px}
  .grade{font-family:var(--mono); font-weight:700; font-size:52px; line-height:.9;
    letter-spacing:-.04em; padding:10px 20px; border:2px solid currentColor}
  ...
  .dial{background:var(--bg); padding:16px 17px 15px; border-top:2px solid var(--line)}
  ...
  .state{display:inline-block; font-family:var(--mono); font-size:8.5px; font-weight:700;
    letter-spacing:.18em; text-transform:uppercase; padding:3px 8px; margin-top:9px}
  .state.pass{color:var(--up); background:rgba(61,232,122,.11)}
  .state.warn{color:var(--warn); background:rgba(255,176,0,.13)}
  .state.fail{color:var(--down); background:rgba(255,90,69,.13)}
  .dial .dv{font-family:var(--mono); font-variant-numeric:tabular-nums;
    font-size:23px; font-weight:700; letter-spacing:-.02em; margin:10px 0 5px; color:var(--ink)}
  .dial .rule{font-family:var(--mono); font-size:10px; color:var(--faint); line-height:1.55}
  ...
  .hnote{font-family:var(--mono); font-size:9.5px; letter-spacing:.06em;
    color:var(--faint); margin:0 0 10px; opacity:.8}
  .hist{display:flex; gap:1px; overflow-x:auto; background:var(--line);
    border:1px solid var(--line); padding:0; scrollbar-width:thin}
  .hist::-webkit-scrollbar{height:6px}
  .hist::-webkit-scrollbar-thumb{background:var(--line)}
  .gcard{flex:0 0 auto; min-width:80px; background:var(--bg); border:0; cursor:pointer;
  ...
  .gw{font-family:var(--mono); font-size:9px; letter-spacing:.11em;
    text-transform:uppercase; color:var(--faint)}
  .gg{font-family:var(--mono); font-size:22px; font-weight:700; letter-spacing:-.03em;
    line-height:1.15; margin:4px 0 2px}
  .gr{font-family:var(--mono); font-size:10px; font-variant-numeric:tabular-nums}
  .gnow{font-family:var(--mono); font-size:7.5px; letter-spacing:.18em;
    text-transform:uppercase; color:var(--accent); margin-top:3px}
  .grecon{font-family:var(--mono); font-size:7.5px; letter-spacing:.14em;
    text-transform:uppercase; color:var(--faint); margin-top:3px}

  /* ---------- heatmap ---------- */
  .heat{display:grid; grid-template-columns:repeat(auto-fill,minmax(15px,1fr));
    gap:2px; margin-top:12px}
  .heat i{display:block; aspect-ratio:1; background:var(--panel-2);
    border:1px solid transparent; cursor:default; transition:transform .1s, border-color .1s}
  .heat i:hover{border-color:var(--ink); transform:scale(1.18); position:relative; z-index:2}
  ...
  /* ---------- allocation bars ---------- */
  .bookgrid{display:grid; grid-template-columns:repeat(auto-fit,minmax(320px,1fr)); gap:34px}
  ...
  .bar .track{height:16px; background:var(--panel-2); border:1px solid var(--line);
  ...
  /* ---------- tables: dense, institutional ---------- */
  .tablewrap{overflow-x:auto; border:1px solid var(--line); margin-top:12px}
  table{width:100%; border-collapse:collapse; font-family:var(--mono); font-size:11.5px}
  thead th{background:var(--panel-2); color:var(--accent); font-weight:700;
  ...
  td{padding:8px 12px; border-bottom:1px solid var(--line); color:var(--muted);
    font-variant-numeric:tabular-nums}
  tbody tr:last-child td{border-bottom:0}
  tbody tr:hover td{background:var(--panel-2); color:var(--ink)}
  th.r,td.r,.num.r{text-align:right}
  .tk{color:var(--ink); font-weight:700; letter-spacing:.06em}
  .chip{display:inline-block; font-family:var(--mono); font-size:8.5px;
    letter-spacing:.15em; text-transform:uppercase; padding:2px 7px;
    border:1px solid var(--line); color:var(--faint)}
  .chip.wheel{color:var(--accent); border-color:rgba(255,176,0,.4)}
  .chip.dir{color:var(--compare); border-color:rgba(56,207,232,.35)}
  ...
  .appr{background:var(--bg); padding:20px; display:flex; flex-direction:column}
  .appr h3{font-family:var(--mono); font-size:12.5px; font-weight:700; letter-spacing:.1em;
    text-transform:uppercase; color:var(--ink); margin:0 0 11px; display:flex;
    align-items:baseline; gap:9px; flex-wrap:wrap}
  .appr .idx{color:var(--accent); font-size:10px; letter-spacing:.2em}
  .appr h3 .tag{font-family:var(--mono); font-size:8.5px; letter-spacing:.16em;
    color:var(--faint); border:1px solid var(--line); padding:2px 7px; text-transform:uppercase}
  .appr p{margin:0; font-family:var(--serif); font-size:14px; line-height:1.7; color:var(--muted)}
  ...
  .con{display:grid; grid-template-columns:repeat(auto-fit,minmax(250px,1fr));
    gap:1px; background:var(--line); border:1px solid var(--line); margin-top:14px}
  .con .box{background:var(--bg); padding:17px 18px; border-top:2px solid var(--line)}
  .con .box.ok{border-top-color:var(--up)}
  .con .box.no{border-top-color:var(--down)}
  .con .box h4{font-family:var(--mono); font-size:9px; letter-spacing:.19em;
    text-transform:uppercase; color:var(--faint); margin:0 0 11px}
  .con ul{margin:0; padding-left:16px}
  .con li{font-family:var(--serif); font-size:13.5px; line-height:1.6;
    color:var(--muted); margin:6px 0}

  /* ---------- EDITORIAL: long-form prose ---------- */
  .prose{font-family:var(--serif); font-size:16px; line-height:1.78; color:var(--muted);
  ...
  .proselist li{font-family:var(--serif); font-size:15px; line-height:1.7;
  ...
  .cnav{display:flex; gap:1px; background:var(--line); border:1px solid var(--line);
    overflow-x:auto; scrollbar-width:none; margin-bottom:30px}
  .cnav::-webkit-scrollbar{display:none}
  .cbtn{appearance:none; background:var(--bg); border:0; cursor:pointer; font:inherit;
  ...
  .cbtn.on{background:var(--accent-soft)}
  .cbtn.on b{color:var(--accent)}
  .concept{display:none}
  .concept.active{display:block; animation:pgIn .3s var(--ease-out) both}
  .fsview{display:none}
  .fsview.active{display:block}
  #panel-ideation .concept>.eyebrow{margin-top:22px}
  canvas#fscurve{height:280px}
  .fs-kicker{font-family:var(--serif); font-style:italic; font-size:17px;
    line-height:1.65; color:var(--ink); max-width:64ch; margin:0 0 18px}
  .fs-note{font-family:var(--serif); font-size:14px; line-height:1.7; color:var(--faint);
    max-width:70ch; border-left:2px solid var(--line); padding-left:16px; margin:18px 0}
  .fs-cover{font-family:var(--mono); font-size:9.5px; letter-spacing:.13em;
    text-transform:uppercase; color:var(--faint); margin-top:10px}
  .fs-legend{display:flex; flex-wrap:wrap; gap:7px; margin:16px 0 6px}
  ...
  .fs-search,.fs-q{font-family:var(--mono); font-size:11px; background:var(--panel);
    border:1px solid var(--line); color:var(--ink); padding:8px 11px; width:100%;
    max-width:280px; margin:14px 0}
  ...
  table.fs-corr th{font-weight:700; color:var(--faint); font-size:9px; letter-spacing:.1em;
  ...
  .fs-tier,.fs-untracked{font-family:var(--mono); font-size:9px; letter-spacing:.13em;
    text-transform:uppercase; color:var(--faint)}
  ...
  .vs-flag,.gs-flags span{display:inline-block; font-family:var(--mono); font-size:8px;
    letter-spacing:.11em; text-transform:uppercase; border:1px solid var(--line);
    color:var(--faint); padding:1px 5px; margin:1px 2px 1px 0}
  .vs-flag.warn{color:var(--down); border-color:rgba(255,90,69,.42)}
  ...
  .fs-untracked{color:var(--faint)}
  .vs-score{font-family:var(--mono); font-weight:700; color:var(--accent)}
  ...
  .vs-ins{display:inline-block; font-family:var(--mono); font-size:8.5px;
    letter-spacing:.1em; padding:1px 6px; border:1px solid var(--line); color:var(--faint)}
  .vs-ins.heavy{background:var(--accent); color:#0A0A0B; border-color:var(--accent); font-weight:700}
  .vs-ins.normal{background:var(--panel-2)}
  ...
  .dist .axis{display:flex; justify-content:space-between; font-family:var(--mono);
    font-size:9px; color:var(--faint); margin-top:6px}
  .more{font-family:var(--mono); font-size:9.5px; letter-spacing:.12em;
    text-transform:uppercase; color:var(--faint); margin-top:10px}
  ...
  .statusbar{display:flex; position:fixed; left:0; right:0; bottom:0; z-index:20;
    ...
    padding:7px 14px; font-family:var(--mono); font-size:9px; letter-spacing:.16em;
    text-transform:uppercase; color:var(--faint)}
  ...
  /* ---------- footer ---------- */
  footer{margin-top:56px; padding-top:20px; border-top:3px solid var(--ink)}
  footer p{font-family:var(--mono); font-size:9.5px; letter-spacing:.1em;
    color:var(--faint); line-height:1.9; margin:0 0 8px; max-width:88ch}
  ...
  .stalebar{position:fixed; left:0; right:0; bottom:30px; z-index:40; background:var(--accent);
    color:#0A0A0B; font-family:var(--mono); font-size:10px; letter-spacing:.13em;
    text-transform:uppercase; padding:9px 14px; text-align:center}
  .stalebar button{font:inherit; margin-left:10px; background:none; cursor:pointer;
    border:1px solid rgba(10,10,11,.5); color:inherit; padding:3px 10px}
  ...
  @media (max-width:760px){
    ...
    .grade{font-size:40px; padding:8px 15px}
  ...
  @media (max-width:520px){
    ...
    .mark{font-size:clamp(30px,11vw,44px)}
    .masthead{gap:10px}
  }
</style></head>
<body>
