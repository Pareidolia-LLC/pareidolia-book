...
Adding a fifth means another JSON plus another `.cbtn` — never another tab.
...
| 04 | A Case for Forecast Events | `forecastcase.json` | edited by hand (static study) |
...
Concept 04 is a **static case study**, not a screen or a tracker. It is Laskoskie's deck on
USD/JPY forecast ladders: 82 sessions of daily prices and the regression, rebuilt from the
deck's own chart data and re-fitted independently, plus the Apr 23 and Apr 24, 2026 ladders
transcribed from its screenshots. Nothing refreshes it. Its Limits view states plainly that the
regression uses same-session high and low, so its R² describes a finished day rather than a
forecast; keep that caveat if the study is ever extended.
